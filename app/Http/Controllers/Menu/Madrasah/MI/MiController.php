...
namespace App\Http\Controllers\Menu\Madrasah\MI;
...
{
    public function indexMi(Request $request)
    {
        $kabkota =  MasterKotaKab::where('kode_parent', '=', 33)->get();

        return view(
            'pages.menu.madrasah.mi.index',
            [
                'kabkota' => $kabkota,
            ]
        );
    }
    public function dataMi(Request $request)
    {
        if ($request->ajax()) {
