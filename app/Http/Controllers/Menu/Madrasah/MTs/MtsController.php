...
namespace App\Http\Controllers\Menu\Madrasah\MTs;
...
{
    public function indexMts(Request $request)
    {
        $kabkota =  MasterKotaKab::where('kode_parent', '=', 33)->get();

        return view(
            'pages.menu.madrasah.mts.index',
            [
                'kabkota' => $kabkota,
            ]
        );
    }
    public function dataMts(Request $request)
    {
        if ($request->ajax()) {
